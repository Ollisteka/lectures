...
**БИ** $i=1$ — пропустили внутренний цикл, если рекурсия и была, то она была непосредственная: $A_1 \rightarrow A_1\alpha$. Значит, во всех остальных продукциях вида $A_1 \rightarrow A_m\alpha$ — $m > 1$

**ПИ** После $i-1$-ой итерации внешнего цикла все нетерминалы $A_k$, где $k<i$ стали “чистыми” — в продукциях вида $A_k \rightarrow A_m\alpha$ — $m>k$

> То есть когда мы будем подставлять эти правила в ШИ, то в правой части будут стоять уже обработанные нетерминалы, с которыми всё ок

**ШИ** $A_i \rightarrow A_m\alpha$. 

Пусть $A_m \rightarrow \beta\gamma \in P$

Если $\beta$ начинается с нетерминала $A_k$, то $k >m$.  Потому что иначе получаем неравенство $k <= m < i$, а для всех $k < i$ по ПИ мы уже избавились от левой рекурсии.
